...
spec = do
  describe "Theme type for aeson"
    ...
    $ let theme =
      ...
      in  eitherDecode theme `shouldBe` Right Theme
            { name   = "foo"
            , colors = Map.fromList [("color0", RGBA 24 24 24 1.0)]
            }
  describe "rgbaToHexColor" $ it "should convert rgba color to hex color" $ do
    rgbaToHexColor (RGBA 255 255 255 1.0) `shouldBe` HexColor "ff" "ff" "ff"
    rgbaToHexColor (RGBA 10 10 10 1.0) `shouldBe` HexColor "0a" "0a" "0a"
