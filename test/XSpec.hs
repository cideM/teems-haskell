...
spec = do
  describe "resourceP" $
    ...
    prs resourceP "color1" `shouldBe` Just "color1"
  describe "nameClassP" $ do
    it "should parse *" $ prs (nameClassP ["*"]) "*color4" `shouldBe` Just "*"
    it "should parse *." $
      prs (nameClassP ["*", "*."]) "*.color4" `shouldBe` Just "*."
    it "should parse xterm." $
      prs (nameClassP ["*", "*.", "xterm."]) "xterm.color4" `shouldBe`
      Just "xterm."
    it "should parse xterm.VTE100." $
      prs (nameClassP ["*", "*.", "xterm.VTE100."]) "xterm.VTE100.color4" `shouldBe`
      Just "xterm.VTE100."
  describe "xLineP" $ do
    it "should parse line with wildcard" $
      prs (xLineP ["*"]) "*color1: #000000" `shouldBe` Just "color1"
    it "should not parse line that does not match exactly" $
      prs (xLineP ["*"]) "XTerm*color1: #000000" `shouldBe` Nothing
    it "should parse line with app name" $
      prs (xLineP ["xterm."]) "xterm.color1: #000000" `shouldBe` Just "color1"
    it "should not parse line with wrong app name" $
      prs (xLineP ["foo"]) "xterm*color1: #000000" `shouldBe` Nothing
