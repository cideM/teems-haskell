...
data App = App
  ...
  , _configCreator :: Theme -> Text -> Either TransformExceptionType Text -- ^ A function that returns a an error message or a new config
...
data Theme = Theme
  { name   :: ThemeName -- ^ Name of the theme. No underscore here since aeson will try to look for a key named "name", not "_name". TODO: I'm sure Aeson has renaming
